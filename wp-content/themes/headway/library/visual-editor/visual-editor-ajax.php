...
class HeadwayVisualEditorAJAX {
			...
			/* Handle skin templates */
				...
				if ( is_array($skin_templates) && count($skin_templates) ) {
					...
					foreach ( $skin_templates as $skin_template_name => $skin_template_blocks ) {
						...
						$template = HeadwayLayout::add_template($skin_template_name, $skin_template_blocks);

						$skin_template_block_id_translations = $skin_template_block_id_translations + $template['block-id-translations']; /* Use + rather than array_merge because + preserves numeric keys */
					...
					foreach ( $design_editor_inputs as $element_id => $element_data ) {
						...
						foreach ( $element_data['special-element-instance'] as $instance_id => $instance_properties ) {
							...
							$instance_block_id = end($instance_id_fragments);

							if ( strpos($instance_id, 'block-') === false || !is_numeric($instance_block_id) || !isset($skin_template_block_id_translations[intval($instance_block_id)]) )
								continue;

							/* Remove existing instance key/value pair */
							unset($design_editor_inputs[$element_id]['special-element-instance'][$instance_id]);

							/* Add new instance key/value pair with new block ID */
							$new_block_id = $skin_template_block_id_translations[intval($instance_block_id)];
							$design_editor_inputs[$element_id]['special-element-instance'][str_replace('block-' . $instance_block_id, 'block-' . $new_block_id, $instance_id)] = $instance_properties;
	...
	public static function method_get_layout_blocks_in_json() {
		...
		self::json_encode(HeadwayBlocksData::get_blocks_by_layout($layout));
